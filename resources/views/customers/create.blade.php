...
@section('content')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight mb-4">Create New Customer</h2>
    ...
    <form action="{{ route('customers.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="tel" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('phone') border-red-500 @enderror" id="phone" name="phone" value="{{ old('phone') }}" required>
            @error('phone')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
